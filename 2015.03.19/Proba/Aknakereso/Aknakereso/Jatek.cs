...
namespace Aknakereso
{
    public partial class JatekAblak : Form
    {
        ...
        int elerhetoGombok = 0;

        public JatekAblak()
        {
            InitializeComponent();
            kovetkezoMITalalat = Adatok.VELETLEN.Next(1, Adatok.NEHEZSEGSZAZALEK);
            //panel1.BackgroundImage = new Bitmap(Aknakereso.Properties.Resources.Finder);
            //panel2.BackgroundImage = new Bitmap(Aknakereso.Properties.Resources.Finder);
        ...
        private void JatekAblak_Load(object sender, EventArgs e)
        {
            ...
            elerheto.Text += elerhetoGombok.ToString();
        }
        ...
        {
            int talaltAkna = 0;
            if (kovetkezoMITalalat <= 0 && talaltAkna < aknak.Length)
            {
                kovetkezoMITalalat = Adatok.VELETLEN.Next(1, Adatok.NEHEZSEGSZAZALEK);
                talalt();
                talaltAkna++;
                elerhetoGombok--;
                if (elerhetoGombok < fedoGombok.Length)
                {
                    Adatok.NEHEZSEGSZAZALEK--;
                }
            }

            else if(talaltAkna < aknak.Length)
            {
                kovetkezoMITalalat--;
                nemtalalt();
                elerhetoGombok--;
            ...
            if(talaltAkna == aknak.Length){
                nyertes();
                for (int i = 0; i < Adatok.N; i++){
                    for (int j = 0; j < Adatok.M; j++){
			            fedoGombok[i, j].Enabled = false;
			        }
                }
            }
            elerheto.Text += elerhetoGombok.ToString();
