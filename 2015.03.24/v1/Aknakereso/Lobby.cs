    ...
    public partial class Lobby : Form
    {
        ...
        private void timerKliensAdatFogad_Tick(object sender, EventArgs e)
        {
            if (Adatok.KLIENS.Available > 0)  //Ha valaki küldött üzenetet
            {
                ...
                String clientName = Encoding.UTF8.GetString(adat, 0, length);
        ...
        private void palyaElkuld()
        {
            ...
            {
                tombString += Adatok.AKNAKHELYE[0, i].ToString("D2") + Adatok.AKNAKHELYE[1, i].ToString("D2");
            }
            byte[] adat = Encoding.UTF8.GetBytes(Adatok.N.ToString("D2") + Adatok.M.ToString("D2") + tombString + "neve lenne");
        ...
        private void timerSzerverAdatFogad_Tick(object sender, EventArgs e)
        {
            ...
            {
                byte[] adat = new byte[256];
                int length = Adatok.KAPCSOLAT.Receive(adat);  //Üzenet fogadása
                string uzenet = Encoding.UTF8.GetString(adat, 0, length);  //Stringgé alakítás
                //MessageBox.Show(uzenet);
                //átlápés a játékba :D
                timerSzerverAdatFogad.Enabled = false;
                (new KliensJatekos(uzenet)).Show();
                this.Hide();
            }
        }
    }
