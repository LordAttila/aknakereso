...
namespace Aknakereso
{
    public partial class Lobby : Form
    {
        ...
        {
            if (Adatok.KLIENS.Available > 0)  // Ha valamelyik küldött üzenetet
            {
        ...
        private void palyaElkuld()
        {
            ...
            byte[] data = data = Encoding.UTF8.GetBytes(Adatok.N.ToString("D2") + Adatok.M.ToString("D2") + tombString + "PITYUKA");
            Adatok.KLIENS.Send(data, data.Length, SocketFlags.None);
        }

        private void timerSzerverAdatFogad_Tick(object sender, EventArgs e)
        {
            if (Adatok.KAPCSOLAT.Available > 0)  // Ha valamelyik küldött üzenetet
            {
                byte[] data = new byte[256];
